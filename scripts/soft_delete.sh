...
if [ "$#" -lt 1 ]; then
  echo "usage: scripts/soft_delete.sh RUN_ID_OR_DB_ID [reason]" >&2
  echo "example by id    : scripts/soft_delete.sh 1 misrun" >&2
  echo "example by run_id: scripts/soft_delete.sh 20260704_214356_ja_specification_l0opback_ja misrun" >&2
  exit 1
fi

TARGET="$1"
REASON="${2:-manual_soft_delete}"
